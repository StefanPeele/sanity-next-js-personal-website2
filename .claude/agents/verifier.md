...
19. **A transitioned focus ring reads as a different ring if you sample it too early.**
...
20. **`body.textContent` is not a safe substitute for `innerText`.** It includes `<script>`
...
21. **The object `sanityFetch` returns is frozen.** Assigning to it crashes the Next build
...
22. **Disproving one instance is not disproving the claim — and confirming one instance is
