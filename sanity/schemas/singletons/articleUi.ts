...
export default defineType({
  ...
  fields: [
    ...
    obj('blocks', 'Block headings and labels', [
      ...
      str('backlinksHeading', 'Backlinks heading'), str('citeHeading', 'Cite heading'),
      defineField({ name: 'citeTemplate', title: 'Citation template', type: 'string', description: 'Placeholders: {title} {site} {date} {url}' }),
      str('readNextHeading', 'Read next heading'),
      obj('readNextLabels', 'Read next labels', [str('deeper', 'Go deeper'), str('broader', 'Go broader'), str('apply', 'Apply this')]),
      str('askHeading', 'Ask heading'), str('askPlaceholder', 'Ask placeholder'), str('askButton', 'Ask button'),
      str('commentsHeading', 'Comments heading'), str('noContent', 'Empty body text'),
    ], { group: 'blocks' }),
    defineField({ name: 'reactionsHeading', title: 'Reactions heading', type: 'string', group: 'blocks' }),
    defineField({ name: 'reactions', title: 'Reactions', type: 'array', of: [{ type: 'vocabEntry' }], group: 'blocks', options: { sortable: false } }),
    obj('credibility', 'Credibility labels', [
      vocab('confidence', 'Confidence levels'), vocab('maturity', 'Maturity'), vocab('load', 'Cognitive load'), vocab('reviewStatus', 'Review status'),
