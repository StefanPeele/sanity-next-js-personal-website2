      ...
      <div className="max-w-7xl mx-auto px-6 pt-16 pb-24 grid grid-cols-1 lg:grid-cols-12 gap-16 print:pt-6 print:gap-8 print:px-0">
        ...
        <div className="lg:col-span-8">
          <header className="mb-20 print:mb-8">
            ...
            <h1 className="text-6xl md:text-7xl font-serif font-bold tracking-tight text-white print:text-black leading-none">
            ...
            <ol className="space-y-14 border-l border-stone-800 print:border-gray-300 ml-1 list-none m-0 p-0">
              {experiences.map((job) => (
                <li key={job._id} className="relative pl-8 md:pl-12 group">
                  ...
                  <p className="meta-label text-stone-400 print:text-gray-600 mb-2">
                  ...
                  {job.techStack && job.techStack.length > 0 && (
                    <ul className="flex flex-wrap gap-2 mt-5 list-none m-0 p-0" aria-label="Tools and technologies">
                      {job.techStack.map((t) => (
                        <li key={t} className="px-2 py-1 text-xs font-mono tracking-wide bg-white/5 border border-white/10 text-stone-300 rounded-sm print:border-gray-300 print:text-black">
                          {t}
                        </li>
                      ))}
                    </ul>
        ...
        <aside className="lg:col-span-4 space-y-10">
            ...
            {skillGroups.length === 0 ? (
              ...
              <div className="space-y-6">
                {skillGroups.map((group) => (
                  <div key={group.cat}>
                    <h3 className="meta-label text-stone-400 print:text-gray-600 mb-3">{group.cat}</h3>
                        ...
                        skill.description && skill.description.length > 0 ? (
                          <li key={skill._id} className="w-full">
                            <details className="group/skill rounded-md border border-white/10 bg-black/30 print:border-gray-300 print:bg-transparent open:border-white/25">
                              <summary className={`meta-label cursor-pointer list-none flex items-center justify-between gap-3 px-3 py-2 text-stone-200 print:text-black ${FOCUS} rounded-md`}>
                          ...
                          <li key={skill._id} className="meta-label flex items-center px-3 py-1.5 rounded-md border border-white/10 bg-black/30 text-stone-200 print:border-gray-300 print:bg-transparent print:text-black">
                ...
                <p className="meta-label text-stone-400 print:hidden">
                  {copy.levelsLegend}
                </p>
              </div>
            )}
          ...
          {certifications.length > 0 && (
            <section aria-labelledby="certs-heading" className="px-2">
              ...
              {earned.length > 0 && (
                <>
                  <h3 className="meta-label text-emerald-400 print:text-black mt-4 mb-1">{copy.certStatusLabels.earned}</h3>
                  <ul className="list-none m-0 p-0">{earned.map((c) => <CertRow key={c._id} cert={c} />)}</ul>
                </>
              )}
              {inProgress.length > 0 && (
                <>
                  <h3 className="meta-label text-amber-400 print:text-black mt-4 mb-1">{copy.certStatusLabels.inProgress}</h3>
                  <ul className="list-none m-0 p-0">{inProgress.map((c) => <CertRow key={c._id} cert={c} />)}</ul>
                </>
              )}
              {planned.length > 0 && (
                <>
                  <h3 className="meta-label text-stone-400 print:text-black mt-4 mb-1">{copy.certStatusLabels.planned}</h3>
          ...
          {page?.activeDirective && (
            <section className="px-2" aria-labelledby="directive-heading">
              <h2 id="directive-heading" className="meta-label flex items-center gap-2 text-stone-400 print:text-gray-600 mb-3">
          ...
          <section className="px-2" aria-labelledby="education-heading">
            <h2 id="education-heading" className="meta-label text-stone-400 print:text-gray-600 mb-3">{copy.sectionLabels.education}</h2>
            <ul className="space-y-4 list-none m-0 p-0">
              {education.map((ed) => (
                <li key={ed._id}>
                  ...
                  {ed.endDate && (
                    <p className="meta-label text-stone-400 print:text-gray-600 mt-1">
          ...
          {page?.resumeUrl ? (
            <div className="p-8 border border-dashed border-stone-700 rounded-lg text-center hover:border-stone-400 transition-colors print:hidden">
              <p className="meta-label text-stone-400 mb-4">{copy.sectionLabels.hardCopy}</p>
              <a
                href={`${page.resumeUrl}?dl=Stefan_Peele_Resume.pdf`}
 className={`meta-label inline-block bg-white text-black px-8 py-4 rounded-sm hover:bg-stone-200 transition-colors ${FOCUS}`}
              >
                {copy.downloadLabel} →
              </a>
            </div>
          ) : draft.isEnabled ? (
            <p className="meta-label text-amber-400 px-2 print:hidden">
              {copy.draftHint}
            </p>
          ) : null}

          <p className="meta-label px-2 text-stone-400 print:hidden">
