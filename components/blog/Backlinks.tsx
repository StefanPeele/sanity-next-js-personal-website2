      ...
      <ul className="space-y-3">
        {notes.map((n) => (
          <li key={n._id} className="flex items-start gap-3">
            ...
            {n.status && <span className="font-mono text-[9px] text-stone-500 pt-1">{n.status}</span>}
          </li>
        ))}
        {posts.map((p) => (
        ...
        {library.map((m) => (
          <li key={m._id} className="flex items-start gap-3">
            ...
            {m.author && <span className="font-mono text-[9px] text-stone-500 pt-1">{m.author}</span>}
          </li>
        ))}
      </ul>
    </section>
