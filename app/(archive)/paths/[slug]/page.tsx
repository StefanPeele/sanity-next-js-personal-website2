    ...
    <div className="relative min-h-screen text-stone-300">
      ...
      <main id="content" className="relative max-w-3xl mx-auto px-6 pt-32 pb-24">
        ...
        <header className="mb-10">
          <div className="flex flex-wrap items-center gap-3 mb-4 font-mono text-[8px] uppercase tracking-[0.3em] text-stone-500">
        ...
        {steps.length > 0 ? (
          ...
          <div className="py-16 text-center border border-white/5 rounded-xl">
            ...
            <p className="font-mono text-[9px] text-stone-500 uppercase tracking-widest">Add posts or notes to it in Sanity Studio → Learning Paths</p>
          </div>
        )}

        <div className="mt-12 pt-8 border-t border-white/5 flex flex-wrap gap-6">
          <Link href="/paths" className={`font-mono text-[10px] uppercase tracking-[0.3em] text-stone-500 hover:text-white transition-colors rounded-sm ${FOCUS}`}>← All paths</Link>
          <Link href="/review" className={`font-mono text-[10px] uppercase tracking-[0.3em] text-stone-500 hover:text-white transition-colors rounded-sm ${FOCUS}`}>Review what you read →</Link>
        </div>
      </main>
    </div>
  )
