...
| **3.5 Status as a filter** | `PENDING35` | **15/15** — `measure-status-filter.mjs`, both states | self | Row is **absent entirely** on published, where no post carries a status. Appears in draft with per-status counts, combines with lane/topic/tag, and offers the *derived* `revised` too |
