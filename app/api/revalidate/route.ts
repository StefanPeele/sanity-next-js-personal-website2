...
//
// IT MOVED HERE ON 2026-09-14, from /api/draft-mode/enable/revalidate. Revalidation has
// nothing to do with enabling draft mode; it was nested under that route only because the
// Sanity starter puts it there. The nesting cost a week. The webhook in Sanity was
// configured with the URL this file now has, /api/revalidate, which did not exist, so every
// publish since 7 September 404'd and nothing on the site said so: a dead webhook does not
// raise an error, it just leaves pages stale, and stale looks exactly like unchanged.
//
// Two checks exist so that cannot repeat. docs/audit/verify-production.mjs probes this route
// on every production sweep and tests/smoke.spec.ts probes it before a deploy. Both send an
// unsigned POST and require 401: 404 means the route moved out from under the webhook again,
