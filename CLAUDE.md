...
- Type sizes come from the named Tailwind steps only — `text-xs` through `text-7xl`, eleven of them, no bracketed one-offs. Nothing renders below 12px anywhere on the site; a mono uppercase micro-label is `.meta-label`, never `font-mono` + `uppercase` + a `tracking-*` of your choosing. `text-lg` (18px) is non-article prose. The one exception is the article heading scale in `components/CustomPortableText.tsx` (h2 32/38, h3 26/30, h4 22/24), which the article owns.
