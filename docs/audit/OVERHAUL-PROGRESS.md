...
| **4.1 / 4.5 Sections and hierarchy** | `PENDING41` | **22/22** — `measure-directory-scale.mjs`, both states, 3 breakpoints | self | Under 12 posts: one grid. At 12+: lead, secondary pair, one section per lane (>=3 posts, newest 6), then a river of rows. Verified at a real 50 via 47 draft-only scale fixtures |
| **4.4 Filters folded** | `PENDING41` | included above | self | Lane row **deleted** — sections subsume it. Topic/tag/status/sort fold behind one `Filters` disclosure beside a search field |
