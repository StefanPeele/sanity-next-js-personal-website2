...
| --- | --- | --- | --- |
| **`73610ee` — the comment delete-revalidation fix** | A genuine Vercel rate limit, this time verified TWO ways: commit status says rate limited *and* no deployment record exists. Production is `31475ac` | n/a — it is real | Time. **Until then, remove a comment by setting its `status`, not by deleting it**: that path carries the document, revalidates correctly, and is the designed one anyway |
